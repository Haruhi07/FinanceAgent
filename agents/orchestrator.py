...
"""
        ...
        while outer_round < max_outer:
            ...
            if last_review_feedback:
                round_user_msg += (
                    ...
                    f"你的下一步决策(根据 issues 类型调不同工具):\n"
                    f"- **缺数据/缺财务/缺新闻** → 调 call_researcher 补数据,传 focus_areas 让它重点查\n"
                    ...
                    f"- **多个 issues** → 一次性调 call_writer,同时传 style_hint 和 length_target\n"
                    f"- 然后必须再调 call_reviewer 验证修改后的文章\n"
                    f"\n## 🚨 硬性规则(2026-08 强制)\n"
    ...
    def _build_orchestrator_system_prompt(self) -> str:
        ...
        return (
            ...
            "- **如果 Reviewer 没通过(passed=false)**:\n"
            "  - 如果 issues 是「缺数据/缺财务/缺新闻」→ 调 call_researcher 补数据(可以传 focus_areas)\n"
            "  - 如果 issues 是「[LENGTH] 字数过多/不足」→ 调 call_writer,**必须传 length_target=Y**(从 issues 解析出目标字数)\n"
            "  - 如果 issues 是「风格/结构/可读性」→ 调 call_writer,**用 style_hint 改 prompt**\n"
            "  - 如果两者都有 → 一次性调 call_writer,同时传 style_hint 和 length_target\n"
